...
history    搜索历史命令    （结合 | grep 使用）
			!命令前缀，自动执行上一次匹配前缀的命令
ctrl r 		输入内容去匹配历史命令

ctrl a 		跳到命令开头
ctrl e  	命令结尾
ctrl 键盘左键     向左跳一个单词  / 右键
...
功能：展示当前工作目录 print work directory
...
- `..`，表示上级目录，比如`../`表示上级目录，`../../`表示上级的上级目录 cd..切换到上一级目录 
...
- 参数：被创建文件夹的路径
- 选项：-p，可选，表示创建前置路径 自动创建不存在的父目录，适用于创建连续多层级的目录
> 注意： 创建文件夹需要修改权限，请确保操作均在HOME目录内，不要再HOME外操作
涉及到权限问题，HOME外无法成功
...
> 参数文件路径，可以作为==管道符的输入==
>
> grep  -E  “code|stock”  filepath   —多个关键字
